...
public class SwitchCoreConfig
{
	...
	private boolean isConfigOld()
	{
		...
		if(input != null)
		{
			...
			logger.i(SwitchCore.getInstance().getInstance().PREFIX + "&7Internal Config Version: &6" + config.getInt("version", 0));
			logger.i(SwitchCore.getInstance().getInstance().PREFIX + "&7Local Config Version: &6" + getConfig().getInt("version", -1));
	...
	private YamlConfiguration writeInternalConfig() throws IOException
	{
		...
		if (file.exists())
		{
			...
			logger.i(SwitchCore.getInstance().prefix() + "&7Old configuration renamed to &6" + fileName);
		}
		...
		if(input != null)
		{
			...
			try {
				...
				logger.i(SwitchCore.getInstance().prefix() + "&7New config created, version: &6" + config.getInt("version"));
			} catch (IOException e)
			{
				logger.i(SwitchCore.getInstance().prefix() + "&cError writing configuration!");
				e.printStackTrace();
			}
