...
public class LilypadConnection implements NetworkConnection
{
	...
	{
		connection = SwitchCore.getPlugin().getConnect();
	...
	public void connect()
	{
		...
		final YamlConfiguration config = SwitchCore.getConfig();
		final Logger logger = SwitchCore.getLogger();
		final SwitchCorePlugin plugin = SwitchCore.getPlugin();
		...
		if(connection != null && !connection.isConnected()) {
			...
			new Thread(new Runnable() {
				...
				public void run()
				{
					...
					SwitchCoreListener.onNetworkConnected();
				}
	...
	private Result sendRequest(Request request) {
		try {
			return connection.request(request).awaitUninterruptibly(SwitchCore.getConfig().getInt("request-timeout", 500));
