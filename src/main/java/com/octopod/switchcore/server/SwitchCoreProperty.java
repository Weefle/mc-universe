 ...
 */
public enum SwitchCoreProperty implements ServerProperty
{
	NAME(new ServerProperty<String>()
	{
		...
		{
			YamlConfiguration config = SwitchCore.getInstance().getConfig();
			String name = config.getString("name");
			return name == null || name.equals("") ? SwitchCore.getInstance().getServerIdentifier() : name;
		}
	}),	
	...
	MAX_PLAYERS(new ServerProperty<Integer>()
	{
		...
		{
			return SwitchCore.getInstance().getInterface().getMaxPlayers();
		}
	}),
	...
	WHITELIST_ENABLED(new ServerProperty<Boolean>()
	{
		...
		{
			return SwitchCore.getInstance().getInterface().getWhitelistEnabled();
		}
	}),
	...
	WHITELISTED_PLAYERS(new ServerProperty<List<String>>()
	{
		...
		public List<String> nextValue()
		{
			return new ArrayList<>(Arrays.asList(SwitchCore.getInstance().getInterface().getWhitelistedPlayers()));
		}
	}),
	...
	private SwitchCoreProperty(ServerProperty<?> handle)
	{
		this.handle = handle;
	}
